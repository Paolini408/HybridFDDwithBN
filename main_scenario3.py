...
from utils.carpet_plot_fault_isolation import create_carpet_matrix_new, plot_single_carpet_matrix

### SCENARIO 3: GROUND TRUTH ###
if __name__ == "__main__":
    ...
    colors_det = {'Normal old': 0, 'Normal new': 0, 'Fault': 1}  # Only for Scenario 3
    labels_det = ['Normal', 'Fault']
    label_column_det = 'label_detection'  # Scenario 3
    ...
    for file_path in path_file_list:
        ...
        output_file = os.path.basename(file_path)
        output_file_path = os.path.join(f'./data/fault_isolated_data/scenario{n_scenario}', output_file)
        df_tot.to_csv(output_file_path, index=False)
    ...
    # Carpet plot for fault detection and isolation
    folder_path = './data/fault_isolated_data/scenario3'
    datetime_column = 'Datetime'
    dataframes = load_data(folder_path, datetime_column)
    num_days = 7 * 12
    xticks_interval = 7
    xticks_new = range(0, num_days * 24, xticks_interval * 24)
    colors_iso = {'Normal': 0, 'Fault CC': 0.25*3/2, 'Fault ECO': 0.25*5/2, 'Fault SAT bias': 1}
    labels_iso = ['Normal', 'Fault CC', 'Fault ECO', 'Fault SAT sensor']
    colormap_iso = ListedColormap(['forestgreen', 'skyblue', 'tomato', 'mediumslateblue'])
    label_column_iso = 'label_isolation'
    carpet_matrix_tot = create_carpet_matrix_new(dataframes, colors_iso, label_column_iso)
    plot_single_carpet_matrix(carpet_matrix_tot, file_names, xticks_new, xtick_labels, labels_iso, colormap_iso)

    colors_iso_pred = {'Normal': 0, 'Fault CC': 0.2, 'Fault ECO': 0.4, 'Fault SAT bias': 0.6, 'Fault Air duct': 0.8, 'Fault MAT bias': 1}
    labels_iso = ['Normal', 'Fault CC', 'Fault ECO', 'Fault SAT sensor', 'Fault Air duct', 'Fault MAT sensor']
    colormap_iso = ListedColormap(['forestgreen', 'skyblue', 'tomato', 'mediumslateblue', 'darkorange', 'navy'])
    label_column_iso_pred = 'label_isolation_pred'
    carpet_matrix_tot = create_carpet_matrix_new(dataframes, colors_iso_pred, label_column_iso_pred)
    plot_single_carpet_matrix(carpet_matrix_tot, file_names, xticks_new, xtick_labels, labels_iso, colormap_iso)
